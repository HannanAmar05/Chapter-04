...
export const SearchResult = () => {
    ...
    const fetchData = async (q) => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_SERVER}3/search/movie?query=${q}`,
          {
            params: {
              query: query,
              include_adult: false,
  ...
  return (
    ...
    <div className='flex items-center justify-center text-white text-2xl py-2'><h1>SEARCH RESULT</h1></div>
    <div className='grid grid-cols-5'>
    {searchResult.map((movie)=> [
      
        <div className='my-6' key={movie.id}>
            <img className='w-11/12 rounded-t-md' src={`https://image.tmdb.org/t/p/original/${movie.poster_path}`} alt="wowo" />
            <div className='bg-white w-11/12 py-4 rounded-b-md flex flex-col px-4 '>
              ...
              <h4 className={`${movie.vote_average >= 8 ? "text-green-500" : movie.vote_average >=6 ? "text-orange-300" : "text-red-600"}`}>{movie.vote_average.toFixed(1)}</h4>
            </div>
              </div>
